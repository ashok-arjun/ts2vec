...
PRETRAIN_SUFFIXES=("2004")


END_DATE="2015-08-15 15:00:00"
...
do
    RUN_NAME_1="RF_Pretraining_from_${PRETRAIN_SUFFIXES[$i]}_to_2015"
    echo $RUN_NAME_1

    python -u train.py neighbour "${RUN_NAME_1}" --loader forecast_csv \
    --repr-dims 320 --max-threads 8 --seed 42 --max-train-length 201  \
    --train --load_feats --method cost --tags 'pretraining' 'new-2' \
    --start_date "${PRETRAIN_START_DATES[$i]}" --end_date "${END_DATE}" \
    ...
    for i in ${!TRAIN_SLICE_END[@]}; do
        ...
        RUN_NAME_2="RF_Pretraining_from_${PRETRAIN_SUFFIXES[$i]}_to_2015_then_train_again_with_CoST_until_${SUFFIXES[$i]}"
        echo $RUN_NAME_2
